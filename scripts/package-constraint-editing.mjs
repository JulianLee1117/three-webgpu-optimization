...
assert.ok(args.length===0||(args.length===1&&args[0]==='--zip'),'Usage: node scripts/package-constraint-editing.mjs [--zip]');
const root=await realpath(path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..'));
const experiment='experiments/constraint-editing/';
const createdAt=new Date().toISOString();
const destinationName='constraint-editing-demo-'+createdAt.replaceAll(':','-');
...
const documents=[];
for(const name of ['README.md','PRIOR_ART.md','PROTOCOL.md','CONSTRAINED-PROTOCOL.md','GPU-PROTOCOL.md','RESULTS.md']){
...
const htmlSource=(await source(experiment+'index.html')).toString();
assert.equal((htmlSource.match(/<script\s+type="importmap">/g)??[]).length,1,'Unexpected import map layout');
assert.equal((htmlSource.match(/src="\/experiments\/constraint-editing\/app\.js"/g)??[]).length,1,'Unexpected app entry');
let html=htmlSource.replace(/\s*<script\s+type="importmap">[\s\S]*?<\/script>/,'').replace('src="/experiments/constraint-editing/app.js"','src="./app.js"');
for(const {source,file}of documents)html=html.replaceAll('href="/'+source+'"','href="./'+file+'"');
html=html.replaceAll('href="/experiments/constraint-editing/README.md"','href="./README.md"');
...
add('THIRD_PARTY_NOTICES.txt','Includes Three.js 0.185.1 under the MIT license; see THREE-LICENSE.txt. Bundle legal comments are retained. esbuild is a packaging tool, not a runtime dependency.\n');
...
export async function startServer(port=5196){
...
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){
  const args=process.argv.slice(2);let value=process.env.PORT??'5196';
  if(args.length){if(args.length===1&&args[0].startsWith('--port='))value=args[0].slice(7);else if(args.length===2&&args[0]==='--port')value=args[1];else throw Error('Usage: node serve.mjs [--port=5196]');}
...
`);
add('README.md',`# Editable native Three shader motion

With Node.js 22.12 or newer, run \`node serve.mjs\` in this extracted folder and open http://127.0.0.1:5196/. No npm install, CDN, model download or API key is needed by this bundled runtime. The app remains idle until **Open experiment** allocates the GPU. Use **Stop** to release it; hiding the page also stops it. Playback is bounded.

Use \`node serve.mjs --port=5197\` or the PORT environment variable to select another loopback port. Press Ctrl+C to stop the server. No browser opens automatically.

This is a restricted pure-native-TSL constraint editing prototype. Editing can be infeasible; sampled obstacle checks are not a continuous collision guarantee. The packaged source keeps one Three.js 0.185.1 source-module runtime, including the private node representation used by the derivative compiler. No new optimization algorithm or performance advantage is claimed.
...
Run \`node verify.mjs\` to verify all packaged file hashes using only Node built-ins. [bundle.metafile.json](bundle.metafile.json) and [manifest.json](manifest.json) record the resolved source modules and input hashes. Three retains its [MIT license](THREE-LICENSE.txt).
`);
...
const manifest={kind:'constraint-editing-portable-v1',createdAt,three:three.version,esbuild:esbuildVersion,
