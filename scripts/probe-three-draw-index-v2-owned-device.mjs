...
const buildRoot = path.join(repoRoot, '.generated', 'three-draw-index-v2-live-build');
...
export function prepareExampleSource(source, buildUrlRoot) {
  ...
  requireCondition(occurrences(body, 'window.__drawIndexV2State.ready = {') === 1, 'Ready hook injection failed.');
  return body;
}
...
function installPageInstrumentation() {
  ...
  Object.defineProperty(window, '__drawIndexV2State', { value: state });
  ...
  window.__drawIndexV2Snapshot = async () => {
    ...
    for (const record of shaderModules) {
      ...
      modules.push({ code: record.code, label: record.label, messages });
    }
    return { ...state, shaderModules: modules };
  };
  window.__drawIndexV2OwnsDevice = (device) => activeDevice !== null && device === activeDevice;
}
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    kind: 'three-webgpu-draw-index-v2-owned-device-report',
    status: 'running',
  ...
  try {
    ...
    browser.on('disconnected', () => {
      if (cleanupStarted === false) observations.browserDisconnects.push('unexpected');
    });
    ...
    const buildUrlRoot = '/.generated/three-draw-index-v2-live-build';
    ...
    await page.waitForFunction(
      () => window.__drawIndexV2State?.ready !== null,
      { timeout: 60_000 },
    );
    await withDeadline(
      page.evaluate(() => window.__drawIndexV2Snapshot()),
      SNAPSHOT_TIMEOUT_MS,
      'Initial evidence settlement',
    );
    ...
    const validation = validateEvidence(evidence, protocol, observations);
