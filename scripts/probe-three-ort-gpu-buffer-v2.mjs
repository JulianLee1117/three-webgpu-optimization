...
const liveBuildRoot = path.join(
  repoRoot,
  '.generated',
  'three-ort-gpu-buffer-canary-v2-ort-1.30.0-dev.20260904-d47fd8824-live-build',
);
const preflightBuildRoot = path.join(
  repoRoot,
  '.generated',
  'three-ort-gpu-buffer-canary-v2-ort-1.30.0-dev.20260904-d47fd8824-preflight-build',
);
const protocolPath = path.join(repoRoot, 'protocols', 'three-ort-gpu-buffer-canary-v2.json');
const ortPackageRoot = path.join(repoRoot, 'node_modules', 'onnxruntime-web-dev');
const stableOrtPackageRoot = path.join(repoRoot, 'node_modules', 'onnxruntime-web');
...
const ORT_VERSION = '1.30.0-dev.20260904-d47fd8824';
const ORT_PACKAGE = 'onnxruntime-web';
const ORT_PACKAGE_ALIAS = 'onnxruntime-web-dev';
const ORT_DEPENDENCY_SPEC = `npm:${ORT_PACKAGE}@${ORT_VERSION}`;
const ORT_COMMON_VERSION = '1.30.0-dev.20260813-72e1c9c9b8';
const ORT_FIX_BUNDLE_MARKER = '_OrtCreateWebGpuInstance';
const STABLE_ORT_VERSION = '1.29.0';
const ORT_COMMON_LOCK_PATH =
  'node_modules/onnxruntime-web-dev/node_modules/onnxruntime-common';
const PROTOCOL_KIND =
  'three-ort-webgpu-borrowed-buffer-canary-protocol-v2-ort-1.30.0-dev.20260904-d47fd8824';
const EXECUTION_ID =
  'ort-three-gpu-buffer-31fdc38-v2-ort-1.30.0-dev.20260904-d47fd8824';
const PAGE_KIND =
  'three-webgpu-onnxruntime-borrowed-gpu-buffer-correctness-canary-v2-ort-1.30.0-dev.20260904-d47fd8824';
const INSTRUMENTATION_KIND =
  'three-ort-gpu-buffer-instrumentation-v2-ort-1.30.0-dev.20260904-d47fd8824';
const REPORT_KIND =
  'three-ort-webgpu-borrowed-buffer-canary-report-v2-ort-1.30.0-dev.20260904-d47fd8824';
const PAGE_PATH = 'three-ort-gpu-buffer-canary-v2.html';
const PAGE_MODULE_PATH = 'src/three-ort-gpu-buffer-canary-v2.js';
const PAGE_CONTRACT_PATH = 'src/three-ort-gpu-buffer-canary-v2-contract.js';
const OUTPUT_DIRECTORY =
  'results/development/three-ort-gpu-buffer-canary-v2/'
    + 'ort-three-gpu-buffer-31fdc38-v2-ort-1.30.0-dev.20260904-d47fd8824';
const SCREENSHOT_FILENAME =
  'ort-three-shared-gpu-buffer-v2-ort-1.30.0-dev.20260904-d47fd8824.png';
...
const BUILD_ROUTE = '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__';
const ORT_ROUTE = '/__ort_webgpu_v2_1.30.0-dev.20260904-d47fd8824__';
...
export async function readOrtRuntimeArtifacts() {
  ...
  const manifestBytes = await readFile(path.join(repoRoot, 'package.json'));
  ...
  const lockEntry = lockJson?.packages?.[`node_modules/${ORT_PACKAGE_ALIAS}`];
  const commonLockEntry = lockJson?.packages?.[ORT_COMMON_LOCK_PATH];
  const stablePackageBytes = await readFile(path.join(stableOrtPackageRoot, 'package.json'));
  const stablePackageJson = JSON.parse(stablePackageBytes.toString('utf8'));
  const stableBundleBytes = await readFile(path.join(
    stableOrtPackageRoot,
    ORT_RUNTIME_FILES['ort.webgpu.bundle.min.mjs'],
  ));
  const files = {};
  const modules = {};
  ...
  return {
    commonLock: {
      integrity: commonLockEntry?.integrity ?? null,
      resolved: commonLockEntry?.resolved ?? null,
      version: commonLockEntry?.version ?? null,
    },
    files,
    lock: {
      bytes: lockBytes.length,
      commonDependencyVersion: lockEntry?.dependencies?.['onnxruntime-common'] ?? null,
      integrity: lockEntry?.integrity ?? null,
      name: lockEntry?.name ?? null,
    ...
    modules,
    markers: {
      [ORT_FIX_BUNDLE_MARKER]: modules['ort.webgpu.bundle.min.mjs']
        .includes(Buffer.from(ORT_FIX_BUNDLE_MARKER, 'utf8')),
    },
    manifest: {
      bytes: manifestBytes.length,
      dependencySpec: manifestJson?.dependencies?.[ORT_PACKAGE_ALIAS] ?? null,
    ...
    stableBaseline: {
      bundle: {
        bytes: stableBundleBytes.length,
        markerPresent: stableBundleBytes.includes(Buffer.from(ORT_FIX_BUNDLE_MARKER, 'utf8')),
        sha256: sha256(stableBundleBytes),
      },
      package: {
        bytes: stablePackageBytes.length,
        name: stablePackageJson.name,
        sha256: sha256(stablePackageBytes),
        version: stablePackageJson.version,
      },
    },
  };
}
...
export function verifyOrtRuntimeArtifacts(runtime, expectedFiles = null, expectedPackage = null) {
  ...
  check(runtime?.package?.name === ORT_PACKAGE, `Runtime package name is not ${ORT_PACKAGE}.`);
  check(runtime?.package?.version === ORT_VERSION, `Runtime package is not exactly ${ORT_VERSION}.`);
  check(runtime?.manifest?.dependencySpec === ORT_DEPENDENCY_SPEC,
    `package.json does not pin ${ORT_PACKAGE_ALIAS} to ${ORT_DEPENDENCY_SPEC}.`);
  check(runtime?.lock?.version === ORT_VERSION,
    `package-lock.json does not resolve ${ORT_PACKAGE_ALIAS} exactly ${ORT_VERSION}.`);
  check(runtime?.lock?.name === ORT_PACKAGE,
    `package-lock.json alias ${ORT_PACKAGE_ALIAS} does not resolve canonical ${ORT_PACKAGE}.`);
  check(runtime?.lock?.commonDependencyVersion === ORT_COMMON_VERSION,
    'Aliased ORT package does not request the exact pinned ORT common version.');
  check(runtime?.commonLock?.version === ORT_COMMON_VERSION,
    `package-lock.json does not resolve the aliased ORT common package exactly ${ORT_COMMON_VERSION}.`);
  check(runtime?.markers?.[ORT_FIX_BUNDLE_MARKER] === true,
    `Nightly ORT bundle lacks required ${ORT_FIX_BUNDLE_MARKER} fix marker.`);
  check(runtime?.stableBaseline?.package?.name === ORT_PACKAGE
      && runtime?.stableBaseline?.package?.version === STABLE_ORT_VERSION,
  `Stable baseline is not exact ${ORT_PACKAGE}@${STABLE_ORT_VERSION}.`);
  check(runtime?.stableBaseline?.bundle?.markerPresent === false,
    `Stable ${STABLE_ORT_VERSION} baseline unexpectedly contains ${ORT_FIX_BUNDLE_MARKER}.`);
  if (expectedPackage !== null) {
    check(expectedPackage.package === ORT_PACKAGE,
      'Protocol canonical ORT package name differs.');
    check(expectedPackage.packageAlias === ORT_PACKAGE_ALIAS,
      'Protocol ORT package alias differs.');
    check(expectedPackage.dependencySpec === ORT_DEPENDENCY_SPEC,
      'Protocol ORT alias dependency spec differs.');
    check(runtime?.lock?.integrity === expectedPackage.lockIntegrity,
      'package-lock.json ORT integrity differs from the protocol.');
    check(expectedPackage.commonPackage === 'onnxruntime-common',
      'Protocol ORT common package name differs.');
    check(expectedPackage.commonVersion === ORT_COMMON_VERSION,
      'Protocol ORT common version differs.');
    check(runtime?.commonLock?.integrity === expectedPackage.commonLockIntegrity,
      'package-lock.json aliased ORT common integrity differs from the protocol.');
    check(exactArray(expectedPackage.requiredBundleMarkers, [ORT_FIX_BUNDLE_MARKER]),
      'Protocol required nightly bundle marker differs.');
    check(expectedPackage?.stableBaseline?.packageAlias === 'onnxruntime-web'
        && expectedPackage?.stableBaseline?.version === STABLE_ORT_VERSION,
    'Protocol stable ORT baseline identity differs.');
    check(exactArray(
      expectedPackage?.stableBaseline?.absentBundleMarkers,
      [ORT_FIX_BUNDLE_MARKER],
    ), 'Protocol stable baseline absent-marker contract differs.');
    check(JSON.stringify(runtime?.stableBaseline?.bundle && {
      bytes: runtime.stableBaseline.bundle.bytes,
      sha256: runtime.stableBaseline.bundle.sha256,
    }) === JSON.stringify(expectedPackage?.stableBaseline?.bundle),
    'Stable ORT baseline bundle differs from the protocol inventory.');
  }
 ...
 *   __setThreeOrtGpuBufferV2Phase('ort-run-1' | 'three-proof-1' | ...)
 *   __registerThreeOrtGpuBufferV2Target(buffer, { logicalBytes: 24 }) -> numeric id
 *   __collectThreeOrtGpuBufferV2Instrumentation() -> one final serializable snapshot
 */
function installOrtGpuBufferInstrumentation() {
  const state = {
    ...
    kind: 'three-ort-gpu-buffer-instrumentation-v2-ort-1.30.0-dev.20260904-d47fd8824',
    phase: 'bootstrap',
    phaseTransitions: [],
    resources: {
      adapters: [],
  ...
  Object.defineProperty(window, '__threeOrtGpuBufferV2Instrumentation', { value: state });
  ...
  Object.defineProperty(window, '__setThreeOrtGpuBufferV2Phase', {
    value: (phase) => {
  ...
  Object.defineProperty(window, '__registerThreeOrtGpuBufferV2Target', {
  ...
  Object.defineProperty(window, '__threeOrtGpuBufferV2ObjectId', {
    value: (object) => idFor(object, object?.constructor?.name ?? 'object'),
  });

  Object.defineProperty(window, '__collectThreeOrtGpuBufferV2Instrumentation', {
    value: async () => {
...
export function verifyOrtGpuBufferPageSource(source) {
  ...
  check(source.includes("from '/__ort_webgpu_v2_1.30.0-dev.20260904-d47fd8824__/ort.webgpu.bundle.min.mjs'"),
    'Canary does not import the pinned local ORT WebGPU bundle route.');
  check(source.includes("from '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.webgpu.js'"),
    'Canary does not import the frozen local Three WebGPU bundle route.');
  check(source.includes("from '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.tsl.js'"),
    'Canary does not import the frozen local Three TSL bundle route.');
  check(source.includes("from './three-ort-gpu-buffer-canary-v2-contract.js'"),
    'Canary does not import the isolated v2 contract module.');
  check(source.includes('ORT_EXPECTED_VERSION'),
    'Canary does not import the exact ORT nightly version contract.');
  check(/state\.environment\.ortVersion\s*!==\s*ORT_EXPECTED_VERSION/u.test(source),
    'Canary does not fail closed when the browser ORT version differs.');
  ...
  check(!/outputTensor\s*\.\s*dispose\s*\(/u.test(source),
    'Canary source disposes the preallocated output tensor in the v2 nightly proof.');
...
export function verifyOrtGpuBufferHtmlSource(source) {
  const reasons = [];
  const check = (condition, message) => {
    if (!condition) reasons.push(message);
  };
  check(typeof source === 'string' && source.length > 0, 'Canary HTML is empty.');
  check(source.includes('ONNX RUNTIME 1.30.0-dev.20260904-d47fd8824'),
    'Canary HTML does not disclose the exact ORT nightly.');
  check(source.includes(
    '"three/webgpu": '
      + '"/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.webgpu.js"',
  ), 'Canary HTML import map differs from the isolated v2 Three route.');
  check(source.includes('src="/src/three-ort-gpu-buffer-canary-v2.js"'),
    'Canary HTML does not load the isolated v2 page module.');
  check(!source.includes('/__three_ort_bridge__/')
      && !source.includes('/src/three-ort-gpu-buffer-canary.js"'),
  'Canary HTML retains a v1 runtime path.');
  return { reasons, verified: reasons.length === 0 };
}
...
export function validateOrtGpuBufferEvidence(evidence, protocol, observations = {}) {
  ...
  const pageKind = protocol?.page?.resultKind ?? PAGE_KIND;
  ...
  check(environment?.ortVersion === ORT_VERSION,
    `Page did not execute exact ORT nightly ${ORT_VERSION}.`);
  ...
  check(lifecycle?.outputTensorDisposed === false,
    'The v2 nightly proof disposed its preallocated output tensor.');
  ...
  check(instrumentation?.kind === INSTRUMENTATION_KIND,
...
export function verifyProtocolHarnessConstants(protocol) {
  const reasons = [];
  const check = (condition, message) => {
    if (!condition) reasons.push(message);
  };
  check(protocol?.schemaVersion === 1, 'Protocol schemaVersion is not 1.');
  check(protocol?.protocolVersion === 2, 'Protocol version is not exactly 2.');
  check(protocol?.kind === PROTOCOL_KIND, 'Protocol kind differs from the v2 nightly kind.');
  check(protocol?.executionId === EXECUTION_ID,
    'Protocol execution ID differs from the v2 nightly execution.');
  check(protocol?.command === 'npm.cmd run probe:three-ort-gpu-buffer-v2 -- --browser '
      + '"C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe" '
      + '--expected-research-commit <40-hex-research-commit>',
  'Protocol browser command differs from the v2 nightly command.');
  check(protocol?.offlinePreflightCommand
      === 'npm.cmd run probe:three-ort-gpu-buffer-v2 -- --offline-preflight',
  'Protocol offline preflight command differs from the v2 nightly command.');
  ...
  check(protocol?.onnxRuntime?.package === ORT_PACKAGE,
    'Protocol ORT package name differs.');
  check(protocol?.onnxRuntime?.packageAlias === ORT_PACKAGE_ALIAS,
    'Protocol ORT package alias differs.');
  check(protocol?.onnxRuntime?.dependencySpec === ORT_DEPENDENCY_SPEC,
    'Protocol ORT dependency spec differs.');
  check(protocol?.onnxRuntime?.version === ORT_VERSION, 'Protocol ORT version differs.');
  check(protocol?.onnxRuntime?.commonPackage === 'onnxruntime-common',
    'Protocol ORT common package name differs.');
  check(protocol?.onnxRuntime?.commonVersion === ORT_COMMON_VERSION,
    'Protocol ORT common version differs.');
  check(/^sha512-[A-Za-z0-9+/]+={0,2}$/u.test(protocol?.onnxRuntime?.lockIntegrity ?? ''),
    'Protocol ORT lockfile integrity pin is invalid.');
  check(/^sha512-[A-Za-z0-9+/]+={0,2}$/u.test(
    protocol?.onnxRuntime?.commonLockIntegrity ?? '',
  ), 'Protocol ORT common lockfile integrity pin is invalid.');
  ...
  check(protocol?.page?.resultKind === PAGE_KIND, 'Protocol page kind differs.');
  check(protocol?.page?.path === PAGE_PATH, 'Protocol page path differs.');
  check(protocol?.page?.module === PAGE_MODULE_PATH, 'Protocol page module differs.');
  check(protocol?.page?.contract === PAGE_CONTRACT_PATH, 'Protocol page contract differs.');
  check(protocol?.output?.directory === OUTPUT_DIRECTORY,
    'Protocol output directory differs from the single-use v2 nightly namespace.');
  check(protocol?.output?.report === 'report.json', 'Protocol report filename differs.');
  check(protocol?.output?.screenshot === SCREENSHOT_FILENAME,
    'Protocol screenshot filename differs.');
  check(protocol?.output?.exclusiveDirectory === true && protocol?.output?.overwrite === false,
    'Protocol does not require an exclusive, non-overwriting output directory.');
  check(protocol?.target?.physicalBytes === 32 && protocol?.target?.logicalBytes === 24,
    'Protocol target size differs from the v2 nightly proof.');
  ...
  'Protocol run vectors differ from the exact v2 vectors.');
  return { reasons, verified: reasons.length === 0 };
}
...
async function offlinePreflight() {
  ...
  const htmlValidation = verifyOrtGpuBufferHtmlSource(
    pageArtifacts.html.bytes.toString('utf8'),
  );
  requireCondition(htmlValidation.verified, htmlValidation.reasons.join(' '));
  ...
  console.log(JSON.stringify({
    ...
    protocolValidation,
    runtime: {
      commonLock: runtime.commonLock,
      files: runtime.files,
      lock: runtime.lock,
      manifest: runtime.manifest,
      markers: runtime.markers,
      package: runtime.package,
      stableBaseline: runtime.stableBaseline,
    },
    validation: {
...
async function browserCanary({ browserPath, expectedResearchCommit }) {
  ...
  const htmlSourceValidation = verifyOrtGpuBufferHtmlSource(
    pageArtifacts.html.bytes.toString('utf8'),
  );
  requireCondition(htmlSourceValidation.verified, htmlSourceValidation.reasons.join(' '));
  const runtime = await readOrtRuntimeArtifacts();
  const runtimeValidation = verifyOrtRuntimeArtifacts(
    runtime,
    protocol.onnxRuntime.files,
  ...
  const report = {
    ...
    pageSourceValidation: {
      html: htmlSourceValidation,
      module: pageSourceValidation,
    },
  ...
  try {
    ...
    server = await createServer({
      ...
      plugins: [{
        ...
        name: 'strict-ort-three-canary-v2-nightly-artifacts',
      }],
      root: repoRoot,
      server: {
    ...
    await page.waitForFunction(
      (screenshotStage) => {
        const status = window.__threeOrtGpuBufferV2Canary?.status;
        return status === screenshotStage
          || status === 'failed'
          || (window.__threeOrtGpuBufferV2Instrumentation?.windowErrors?.length ?? 0) > 0
          || (window.__threeOrtGpuBufferV2Instrumentation?.unhandledRejections?.length ?? 0) > 0;
      },
    ...
    const readyPage = await page.evaluate(() => (
      structuredClone(window.__threeOrtGpuBufferV2Canary)
    ));
    const readyInstrumentation = await page.evaluate(() => (
      structuredClone(window.__threeOrtGpuBufferV2Instrumentation)
    ));
    ...
    await page.evaluate(() => {
      void window.__continueThreeOrtGpuBufferV2Canary();
    });
    await page.waitForFunction(
      () => ['passed', 'failed'].includes(window.__threeOrtGpuBufferV2Canary?.status),
      undefined,
      { timeout: protocol.page.timeoutMilliseconds ?? DEFAULT_PAGE_TIMEOUT_MS },
    );
    ...
    const instrumentation = await withDeadline(
      page.evaluate(() => window.__collectThreeOrtGpuBufferV2Instrumentation()),
