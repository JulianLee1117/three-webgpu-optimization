import * as ort from '/__ort_webgpu_v2_1.30.0-dev.20260904-d47fd8824__/ort.webgpu.bundle.min.mjs';
import * as THREE from '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.webgpu.js';
import {
  Fn,
  float,
  floatBitsToUint,
  ...
  vec3,
  vec4,
} from '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.tsl.js';
...
} from './three-ort-gpu-buffer-canary-v2-contract.js';

const ORT_WASM_PATH = '/__ort_webgpu_v2_1.30.0-dev.20260904-d47fd8824__/ort-wasm-simd-threaded.asyncify.wasm';
const ORT_BUNDLE_PATH = '/__ort_webgpu_v2_1.30.0-dev.20260904-d47fd8824__/ort.webgpu.bundle.min.mjs';
const THREE_BUNDLE_PATH = '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.webgpu.js';
const THREE_TSL_PATH = '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/three.tsl.js';
...
const state = {
  ...
  environment: {
    ...
    instrumentation: {
      phaseHook: typeof window.__setThreeOrtGpuBufferV2Phase === 'function',
      targetHook: typeof window.__registerThreeOrtGpuBufferV2Target === 'function',
      collector: typeof window.__collectThreeOrtGpuBufferV2Instrumentation === 'function',
...
Object.defineProperty(window, '__threeOrtGpuBufferV2Canary', { value: state });
...
function setProbePhase(phase) {
  ...
  if (typeof window.__setThreeOrtGpuBufferV2Phase === 'function') {
    window.__setThreeOrtGpuBufferV2Phase(phase);
  }
}
...
function targetDestroyCountFromInstrumentation() {
  const instrumentation = window.__threeOrtGpuBufferV2Instrumentation;
...
function createOracleTarget() {
  ...
  target.texture.name = 'ort-three-v2-nightly-borrowed-buffer-exact-rgba32uint-oracle';
...
async function createThreeProof(runIndex) {
  ...
  try {
    ...
    proof.attribute.name = `ort-three-v2-output-${runIndex + 1}`;
...
async function runCanary() {
  if (state.environment.ortVersion !== ORT_EXPECTED_VERSION) {
    throw new Error(
      `Expected exact ONNX Runtime Web ${ORT_EXPECTED_VERSION}; loaded ${state.environment.ortVersion}.`,
    );
  }
  ...
  internals.targetBuffer = internals.device.createBuffer({
    label: 'ort-three-v2-nightly-preallocated-output',
    size: ORT_TARGET_PHYSICAL_BYTES,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
  });
  const registrationResult = typeof window.__registerThreeOrtGpuBufferV2Target === 'function'
    ? window.__registerThreeOrtGpuBufferV2Target(internals.targetBuffer, {
...
Object.defineProperty(window, '__continueThreeOrtGpuBufferV2Canary', {
