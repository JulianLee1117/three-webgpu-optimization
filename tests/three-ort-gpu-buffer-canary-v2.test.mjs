...
import {
  ...
  ndcToOraclePixel,
} from '../src/three-ort-gpu-buffer-canary-v2-contract.js';
import {
  readOrtRuntimeArtifacts,
  validateOrtGpuBufferEvidence,
  verifyOrtGpuBufferHtmlSource,
  verifyOrtGpuBufferPageSource,
  verifyOrtRuntimeArtifacts,
  verifyProtocolHarnessConstants,
} from '../scripts/probe-three-ort-gpu-buffer-v2.mjs';

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const protocolPath = path.join(repoRoot, 'protocols', 'three-ort-gpu-buffer-canary-v2.json');
const pageModulePath = path.join(repoRoot, 'src', 'three-ort-gpu-buffer-canary-v2.js');
const pageHtmlPath = path.join(repoRoot, 'three-ort-gpu-buffer-canary-v2.html');
  ...
  return {
    page: {
      ...
      environment: {
        ...
        deviceAcquired: true,
        expectedOrtVersion: ORT_EXPECTED_VERSION,
        ortVersion: ORT_EXPECTED_VERSION,
...
test('ORT v2 evidence rejects a browser runtime version other than the exact nightly', async () => {
  const protocol = await readProtocol();
  const evidence = syntheticEvidence(protocol);
  evidence.page.environment.ortVersion = '1.29.0';
  assertRejected(protocol, evidence, /exact ORT nightly/iu);
});
...
test('ORT canary page source verifier enforces the GPU-only read-only contract', async (t) => {
  ...
  for (const scenario of [
    ...
    {
      label: 'nightly version guard bypass',
      source: source.replace(
        'state.environment.ortVersion !== ORT_EXPECTED_VERSION',
        'false',
      ),
      reason: /ORT version differs/iu,
    },
    {
      label: 'v1 contract import',
      source: source.replace(
        './three-ort-gpu-buffer-canary-v2-contract.js',
        './three-ort-gpu-buffer-canary-contract.js',
      ),
      reason: /v2 contract/iu,
    },
  ]) {
    await t.test(scenario.label, () => {
...
test('ORT v2 canary HTML is isolated from all v1 runtime paths', async (t) => {
  const source = await readFile(pageHtmlPath, 'utf8');
  const valid = verifyOrtGpuBufferHtmlSource(source);
  assert.equal(valid.verified, true, valid.reasons.join('\n'));

  for (const scenario of [
    {
      label: 'wrong visible runtime version',
      source: source.replace(ORT_EXPECTED_VERSION, '1.29.0'),
      reason: /exact ORT nightly/iu,
    },
    {
      label: 'v1 Three route',
      source: source.replace(
        '/__three_ort_bridge_v2_ort_1.30.0-dev.20260904-d47fd8824__/',
        '/__three_ort_bridge__/',
      ),
      reason: /import map|v1 runtime/iu,
    },
    {
      label: 'v1 page module',
      source: source.replace(
        '/src/three-ort-gpu-buffer-canary-v2.js',
        '/src/three-ort-gpu-buffer-canary.js',
      ),
      reason: /v2 page module|v1 runtime/iu,
    },
  ]) {
    await t.test(scenario.label, () => {
      const result = verifyOrtGpuBufferHtmlSource(scenario.source);
      assert.equal(result.verified, false);
      assert.match(result.reasons.join('\n'), scenario.reason);
    });
  }
...
test('ORT v2 protocol pins the isolated nightly runtime and every evidence namespace', async () => {
  const protocol = await readProtocol();
  assert.equal(protocol.schemaVersion, 1);
  assert.equal(protocol.protocolVersion, 2);
  assert.equal(
    protocol.kind,
  ...
  assert.equal(protocol.onnxRuntime.package, 'onnxruntime-web');
  assert.equal(protocol.onnxRuntime.packageAlias, 'onnxruntime-web-dev');
  assert.equal(
    protocol.onnxRuntime.dependencySpec,
    'npm:onnxruntime-web@1.30.0-dev.20260904-d47fd8824',
  );
  assert.equal(protocol.onnxRuntime.version, ORT_EXPECTED_VERSION);
  assert.equal(protocol.onnxRuntime.commonVersion, '1.30.0-dev.20260813-72e1c9c9b8');
  assert.deepEqual(protocol.onnxRuntime.requiredBundleMarkers, ['_OrtCreateWebGpuInstance']);
  assert.equal(protocol.onnxRuntime.stableBaseline.version, '1.29.0');
  assert.deepEqual(
    protocol.onnxRuntime.stableBaseline.absentBundleMarkers,
    ['_OrtCreateWebGpuInstance'],
  );
  assert.match(protocol.onnxRuntime.route, /v2_1\.30\.0-dev\.20260904-d47fd8824/u);
  assert.match(protocol.build.liveDirectory, /v2-ort-1\.30\.0-dev\.20260904-d47fd8824/u);
  assert.equal(protocol.page.path, 'three-ort-gpu-buffer-canary-v2.html');
  assert.equal(protocol.page.module, 'src/three-ort-gpu-buffer-canary-v2.js');
  assert.equal(protocol.page.contract, 'src/three-ort-gpu-buffer-canary-v2-contract.js');
  assert.match(protocol.output.directory, /v2-ort-1\.30\.0-dev\.20260904-d47fd8824/u);
  ...
  assert.match(ORT_GPU_BUFFER_CANARY_KIND, /v2-ort-1\.30\.0-dev\.20260904-d47fd8824/u);
...
test('ORT v2 protocol verifier rejects mixed v1 or unpinned identifiers', async (t) => {
  const protocol = await readProtocol();
  const valid = verifyProtocolHarnessConstants(protocol);
  assert.equal(valid.verified, true, valid.reasons.join('\n'));

  for (const scenario of [
    {
      label: 'v1 execution ID',
      mutate(value) { value.executionId = 'ort-three-gpu-buffer-31fdc38-v1'; },
      reason: /execution ID/iu,
    },
    {
      label: 'stable package alias',
      mutate(value) { value.onnxRuntime.packageAlias = 'onnxruntime-web'; },
      reason: /package alias/iu,
    },
    {
      label: 'floating nightly dependency',
      mutate(value) { value.onnxRuntime.dependencySpec = 'npm:onnxruntime-web@dev'; },
      reason: /dependency spec/iu,
    },
    {
      label: 'v1 runtime route',
      mutate(value) { value.onnxRuntime.route = '/__ort_webgpu__/'; },
      reason: /ORT route/iu,
    },
    {
      label: 'v1 live build directory',
      mutate(value) {
        value.build.liveDirectory = '.generated/three-ort-gpu-buffer-canary-v1-live-build';
      },
      reason: /live build directory/iu,
    },
    {
      label: 'v1 page module',
      mutate(value) { value.page.module = 'src/three-ort-gpu-buffer-canary.js'; },
      reason: /page module/iu,
    },
    {
      label: 'v1 output directory',
      mutate(value) {
        value.output.directory =
          'results/development/three-ort-gpu-buffer-canary/ort-three-gpu-buffer-31fdc38-v1';
      },
      reason: /output directory/iu,
    },
  ]) {
    await t.test(scenario.label, () => {
      const mutated = structuredClone(protocol);
      scenario.mutate(mutated);
      const result = verifyProtocolHarnessConstants(mutated);
      assert.equal(result.verified, false);
      assert.match(result.reasons.join('\n'), scenario.reason);
    });
  }
...
test('ORT v2 runtime verifier requires the exact alias, nightly, common package, and bytes',
  async (t) => {
    const protocol = await readProtocol();
    const runtime = await readOrtRuntimeArtifacts();
    const valid = verifyOrtRuntimeArtifacts(
      runtime,
      protocol.onnxRuntime.files,
      protocol.onnxRuntime,
    );
    assert.equal(valid.verified, true, valid.reasons.join('\n'));
    assert.equal(valid.inventoryPinned, true);
    ...
    for (const scenario of [
      {
        label: 'manifest points at stable ORT',
        mutate(value) { value.manifest.dependencySpec = '1.29.0'; },
        reason: /package\.json.*alias|does not pin/iu,
      },
      {
        label: 'alias lock resolves stable ORT',
        mutate(value) { value.lock.version = '1.29.0'; },
        reason: /package-lock.*alias|does not resolve/iu,
      },
      {
        label: 'alias loses canonical package identity',
        mutate(value) { value.lock.name = 'onnxruntime-web-dev'; },
        reason: /canonical/iu,
      },
      {
        label: 'transitive common version drifts',
        mutate(value) { value.commonLock.version = '1.29.0'; },
        reason: /common package/iu,
      },
      {
        label: 'relevant nightly fix marker disappears',
        mutate(value) { value.markers._OrtCreateWebGpuInstance = false; },
        reason: /fix marker/iu,
      },
      {
        label: 'stable baseline unexpectedly has fix marker',
        mutate(value) { value.stableBaseline.bundle.markerPresent = true; },
        reason: /baseline.*contains/iu,
      },
      {
        label: 'nightly bundle bytes drift',
        mutate(value) { value.files['ort.webgpu.bundle.min.mjs'].sha256 = '0'.repeat(64); },
        reason: /artifacts differ/iu,
      },
    ]) {
      await t.test(scenario.label, () => {
        const mutated = structuredClone(base);
        scenario.mutate(mutated);
        const result = verifyOrtRuntimeArtifacts(
          mutated,
          protocol.onnxRuntime.files,
          protocol.onnxRuntime,
        );
